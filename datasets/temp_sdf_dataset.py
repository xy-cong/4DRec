...
class TempSdfDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        ...
        if self.rep == 'mesh':
            ...
            for fid in self.fid_list:
                fname = '/'.join(fid.split('-'))
                mesh_raw = trimesh.load(f"{self.raw_mesh_dir}/{fname}.obj", process=False, maintain_order=True)
...
if __name__ == '__main__':
    ...
    train_dataset = TempSdfDataset(mode='train', rep=config.rep, config=config.dataset)
    test_dataset  = TempSdfDataset(mode='test',  rep=config.rep, config=config.dataset)
